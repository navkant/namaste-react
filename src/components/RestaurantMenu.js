  ...
  return (
    <div className="text-center m-4">
      <div>
        ...
        <p>{restInfo.data.cards[2].card.card.info.cuisines.join(", ")}</p>
        {itemCategories.map((category, index) => (
          <ItemCategory
            key={category.categoryId}
            category={category?.card?.card}
            showItems={index === visibleCategoryIndex ? true : false}
            setvisibleCategoryIndex={() => setvisibleCategoryIndex(index)}
          />
        ))}
      </div>
    </div>
